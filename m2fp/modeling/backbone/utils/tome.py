...
def bipartite_soft_matching_random2d(metric, w, h, sx, sy, r, no_rand=False):
    """
...
def compute_merge(x, H, W, tome_info):
    r = int(x.shape[1] * tome_info["ratio"])
    use_rand = False if x.shape[0] % 2 == 1 else tome_info["use_rand"]
    m, u = bipartite_soft_matching_random2d(x, W, H, tome_info["sx"], tome_info["sy"], r, not use_rand)

    m_a, u_a = (m, u)  # if tome_info["merge_attn"] else (do_nothing, do_nothing)

    return m_a, u_a  # Okay this is probably not very good
